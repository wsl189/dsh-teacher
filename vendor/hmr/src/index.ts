...
class Hmr extends Service {
  ...
  constructor(ctx: Context, public config: Hmr.Config) {
    ...
    if (this.internal === undefined && config.root.length > 0) {
      throw new Error('--expose-internals is required when HMR watches module roots')
    }
    this.baseDir = fileURLToPath(new URL(config.base || '.', ctx.baseUrl))
  }

  /** Return the internal loader required by module-reload operations. */
  private requireInternal(): ModuleLoader {
    if (this.internal === undefined) {
      throw new Error('--expose-internals is required for HMR module reload')
    }
