...
The [bundled extensions and QQ speech decision](../feature/2026-08-25-bundled-extensions-and-qq-speech.md) separately authorizes the exact `@huanlin/dsh-plugin-better-sidebar-plugin-office` identity under its declared AGPL-3.0 terms. The generator still classifies AGPL-3.0 as non-permissive, renders the package and an authorization paragraph in the notices, and rejects similarly named forks or any other copyleft runtime. Version changes still require the ordinary dependency, lockfile, compatibility, license, and shipped-client review.
...
The owner-authorization tests prove that only the exact direct Claude SDK and Office viewer identities bypass the ordinary non-permissive-runtime rejection and that neither bypass changes license classification. Claude payload tests additionally prove that the payload set comes from the SDK manifest rather than a version or platform allowlist. Wrong identities, similarly named package forks, missing payloads, and unrelated optional package identities all fail.
...
**Treat authorized terms as permissive or add a wildcard non-permissive allowlist.** Either choice would misstate upstream declarations and let an unrelated runtime inherit authorization it was never granted. Each exception keys one exact direct package identity, remains visibly non-permissive, and links the decision that accepted its obligations; Claude optional payload identities are accepted solely as data declared by that SDK.
