...
PPT Master 提供方在加载的说明后附加当前会话工作区，并要求显式使用 `project_manager.py init --dir` 将项目建在其中，除非用户要求其他获准位置。上游技能文件、资源目录、工作流选择和署名检查保持原样。[Office 清理决策](../feature/2026-09-19-office-generation-cleanup.zh.md)负责临时项目放置，并在最终发布后移除项目内备份。
