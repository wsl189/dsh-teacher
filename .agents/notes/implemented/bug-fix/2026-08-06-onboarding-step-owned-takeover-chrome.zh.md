...
设置外壳在 `settings.onboarding` 有已注册且本地未完成的步骤时，就立即挂出首次使用引导的接管界面框架——portal 到 body 的浮层，带不透明的 `--dsw-alias-bg-layer-1` 展示层、模糊遮罩，并把 `#root` 置为 `inert`。步骤需要先加载自己的私有就绪事实才能判定是否出场，加载期间渲染 `null`。渲染 `null` 无法抑制界面框架，因为不透明展示层是外壳画在 slot outlet 外面的，不属于步骤。
...
`SettingsRoot` 保留协调器（有序账本投影、每次挂载一个步骤、本地完成集合、`stepId`／`complete`／`openSection` 状态），但对当选步骤裸渲染——不再有 portal、展示层和 inert 效果。`settings.onboarding` 的 slot 约定写明：注册方持有外层包裹，且在私有事实未决时必须渲染 `null`。
...
**把 `settings.onboarding` 改成 chain 并把完成集合外置为 store。** composer takeover 的版型；做过原型后回退。selector 只能判定 owner props，私有就绪事实仍然只能在组件内部解析——chain 买来的是当前流程并不需要的路由通用性，代价却是跨三个包的约定变更。
