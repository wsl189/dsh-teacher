...
Before recognition starts, the browser captures the Class, Grade, or Study destination together with its class catalog and current defaults. Every upload first passes through MinerU. Raster images request one enhanced whole-image pass plus overlapping detail regions, while PDF and Office documents use extracted content. The browser evaluates the enhanced overview and the combined detail documents as separate candidates, deduplicates each candidate, keeps the one with more distinct timetable slots, and prefers details on a tie. Within each candidate it expands weekday headings over ascending class runs, chooses a populated period axis instead of a compound corner heading, joins split subject and teacher rows, and unfolds repeated local period sequences in source order. Study classification reads titles inside or outside each table. Class labels with the same grade and terminal class number use the captured catalog name when available and otherwise the first complete spelling in the source, so punctuation differences between morning and evening tables cannot split one class. A non-empty deterministic result goes directly to editable review. Only an empty result starts one text-agent attempt with the MinerU Markdown. The assembled Web profile gives that fallback child five minutes for inspection and repair.
...
Service tests cover tool-model fallback and persistence, child selection, destination personas, compact matrix parsing, joined-keyword canonicalization, retained-draft line repair, tool restriction, accepted-token enforcement, hierarchical class completion, study duty normalization, repeated unnumbered study rows, slot deduplication, oversized input, missing services, and invalid output. OCR tests cover the enhanced whole image and six overlapping image regions. Client tests cover captured destination context, overview/detail candidate selection, compound corner headings, in-table Study titles, class reconciliation across cropped and differently punctuated labels, MinerU-first rule success, agent fallback after an empty rule result, editable review, bulk import, and Remote wiring. Real uploads to the configured MinerU endpoint produce 40 selected entries from the supplied one-class Week image, with eight periods on each weekday; 110 selected entries from the supplied Study image, split into 55 morning and 55 evening assignments across 11 classes; and 440 selected entries from the supplied low-resolution Grade image, with 40 entries for each class and 88 for each weekday. Visible OCR spelling errors remain, so editable review is still required.
