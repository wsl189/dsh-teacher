...
**提供方按次投影，而不是固化。** 内置 AnySearch 适配器会在每次操作入口读取一次 settings 快照，因此变更后的凭据引用、服务地址与结果上限会共同作用于下一次请求，无需重新注册提供方。调用方未指定结果数时，搜索会发送配置的上限；指定时发送两者中的较小值，因此标准、高级与批量调用都不能超过用户设置。可编辑分节开放 `apiKeyEnv`、`baseURL` 与 `maxResults`；`maxRenderedContentChars` 仍由组合配置持有，因为高级工具会在注册时捕获该上限。
...
- **让提供方按属性逐次读取 options。** 在每个使用处读取 thunk 会悄然违背构造函数的承诺：一次操作会先 await 凭据解析，再发送请求，因此落在这段 await 内的 settings 写入可能把按旧分节解析出的密钥发往新分节指定的服务地址。现在每次操作在入口只取一次快照，并把该快照传进凭据解析。
...
用户可以在设置页编辑 shell 的命令超时与输出上限、agent 循环的并行工具调用上限，以及 AnySearch 的可选密钥、服务地址与单次搜索结果上限，每个字段都标注是否由自己设定，并提供重置。
