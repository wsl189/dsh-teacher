...
**提供方按次投影，而不是固化。** 内置 AnySearch 适配器会在每次操作入口读取一次 settings 快照，因此变更后的凭据引用与服务地址会成对作用于下一次请求，无需重新注册提供方。可编辑分节只开放 `apiKeyEnv` 与 `baseURL`；`maxRenderedContentChars` 仍由组合配置持有，因为高级工具会在注册时捕获该上限。
...
用户可以在设置页编辑 shell 的命令超时与输出上限、agent 循环的并行工具调用上限，以及 AnySearch 的可选密钥与服务地址，每个字段都标注是否由自己设定，并提供重置。
