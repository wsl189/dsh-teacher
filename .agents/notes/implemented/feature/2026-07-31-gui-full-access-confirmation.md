...
**Every permission picker gates `danger-full-access` behind the shared in-page `RiskConfirmation` dialog whose enabling action stays disabled until an explicit acknowledgement checkbox is checked; the preset renders under the locale's product label; every dismissal path submits nothing.**
...
- The three built-in presets use locale-owned labels in the composer, `/permission` popup, and Settings row; the two current-session pickers also show localized descriptions. Chinese presents them as `请求批准`, `帮我批准`, and `完全访问权限`. Host-defined values retain their supplied copy, command and Settings writes keep machine names on the wire, and each warning body remains locale-aware in Chinese and English.
