...
第三个同级包 **`dsh-host-directory-picker-auto`**：一个只有 node 半侧的*选择器*，不持有任何选取代码，也没有 UI。它的 `apply` 在启动时恰好采样一次宿主事实——从注入的 `webServer` 读绑定宿主（新增的 `host` getter 与既有的 `port` 对称）、Node.js 是否嵌入 Electron、`SSH_CONNECTION`／`SSH_TTY`、平台、`DISPLAY`／`WAYLAND_DISPLAY`、以及对 Linux 选择器二进制（zenity／kdialog）的一次 `PATH` 探查——经由一个导出的纯函数判定，再用 `ctx.loader.create({name})` 把选中的双面后端挂进 Loader 的**内存根树**；该 effect 的 disposer 会移除该条目并汇入后端 fiber 的拆卸（单靠 `remove()` 只是启动拆卸），因此，只有后端完全停稳后，选择器的卸载才会完成。`native` 要求全部“有人值守且可服务”信号：回环绑定 ∧ 无 SSH 标记 ∧ native 后端能驱动的显示会话——darwin 与独立 win32 Node.js 上视为存在，linux 上要求 `DISPLAY`／`WAYLAND_DISPLAY` 外加一个选择器二进制，其余平台一律不成立（native 后端恰好支持 darwin／win32／linux）。嵌入 Electron 的 win32 进程会判定为 `browse`：浏览交互留在当前桌面应用中，而 win32 native 后端会把 `process.execPath` 重新启动为 COM 对话框 worker，从而再次启动打包后的 Electron 可执行文件。其他任何含糊情形也都判定为处处可用的 `browse`。`apps/cli` 把 `-auto` 挂为它的 `directory-picker` 行；直接组合 `-native` 或 `-browse` 仍是固定交互的方式。
...
- 随附的 web GUI 开箱即自适应：有人值守的本地独立宿主 → OS 选择器；安装版 Electron／Windows 桌面、SSH 启动、全网卡绑定、无头宿主、不支持的平台，或没有选择器二进制的 Linux → 应用内浏览器。探测仍从启动上下文推断操作者位置，而任何启动侧信号都无法证明这一点：脱离的 tmux 会话会丢失 `SSH_*`；非 Aqua 的 darwin 进程仍被算作有显示；而 `ssh -L` 形态（在工作站本地启动、之后经转发端口访问，从 `127.0.0.1` 到达）会在符合条件的独立宿主上判定 `native`，把选择器弹在无人值守的工作站上——即便按连接自适应也修不了最后这一情形。错误的 `native` 选择会退化为后端既有的可重试失败对话框；处于这些形态的部署直接组合 `-browse`。
...
- resolver 单元测试与 REAL Loader 组合测试固定 Electron／win32 选择。Windows 打包产物 workflow 会启动解包后的 Electron 应用，并要求 `host.listDirectory` 返回真实目录列表；无密钥浏览器场景则打开 QQ 机器人工作区操作并固定其应用内目录对话框快照。
