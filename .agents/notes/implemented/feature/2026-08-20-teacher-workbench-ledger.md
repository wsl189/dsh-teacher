...
Daily Management had task, memo, and calendar storage but no place for recurring household expenses such as insurance premiums and utility bills. Reusing memos would lose category ownership, exact currency values, and the occurrence time needed to review those expenses.
...
**Store ledger rows as memos.** This would reuse one data type but would require parsing category, amount, and time from prose, preventing exact totals and referential validation.
