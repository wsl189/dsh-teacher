...
`@deepseek-ai/dsh-ocr-mineru` registers provider id `mineru` and adapts the self-hosted synchronous `/file_parse` API. Its ordinary DSH plugin configuration owns the endpoint, backend, hybrid effort, language, deadline, upload limit, response-byte limit, and Markdown-character limit; the provider and Web/desktop composition default the upload limit to 50 MiB, with a configurable maximum of 100 MiB. A card under **Settings → Plugins → Plugin configuration** exposes every field through the revisioned settings document. The provider accepts PDF, supported raster images, DOCX, PPTX, and XLSX; validates names, canonical base64, extensions, sizes, HTTP status, response bytes, JSON fields, and non-empty Markdown; and never persists source bytes or provider output. An extraction that requests discarded text obtains `middle_json` in the same MinerU call and prepends unique discarded lines that do not already occur in the Markdown.
...
The same input bar owns voice intake. A microphone button toggles one MediaRecorder session, and holding Space for 500 milliseconds starts the same recorder until key release; a shorter Space press remains an ordinary space edit. The complete recording crosses the `speech.transcribe` Remote, and the final transcript enters the input machine at the current selection and stays user-editable. Unsupported browsers disable the voice control, while recording or Host transcription failures use localized composer notices. The [bundled extensions and shared speech decision](2026-08-25-bundled-extensions-and-qq-speech.md) owns the supplier-model provider and supersedes browser-native transcription.
