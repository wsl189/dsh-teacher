...
The Doc skill requires explicit TeX markers and the exporter enforces their conversion. Currency text is not parsed as math. An equation cannot cross a paragraph, field, image, or hyperlink boundary. The live editor retains TeX source; final layout verification reads and renders the exported Word file.
