...
**每个权限选择器都把 `danger-full-access` 关进共享的页面内 `RiskConfirmation` 对话框：启用按钮在用户勾选明确的风险确认复选框前保持禁用；预设使用当前 locale 的产品名称；所有取消路径都不作任何提交。**
...
- 三个内置预设在 composer、`/permission` popup 与 Settings 行中使用 locale 自有的名称，两个当前会话选择器还会显示本地化说明；中文依次显示为 `请求批准`、`帮我批准` 与 `完全访问权限`。host 自定义值保留其提供的文案，命令与 Settings 写入在 wire 上保留机器名，每份警示正文都保持中英文 locale 感知。
