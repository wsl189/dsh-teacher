...
后端会在每次启动受限子进程前检查 `GetConsoleWindow`。没有控制台窗口的宿主请求 `STARTF_USESHOWWINDOW` 与 `SW_HIDE`；拥有控制台的宿主则与子进程共享该控制台。这项显示提示不会改变进程创建标志或 stdio 继承。
...
上述控制台限制中的隐藏控制台创建特指 `CREATE_NO_WINDOW` 或 `CREATE_NEW_CONSOLE`。无控制台 GUI 宿主使用 `STARTUPINFOW` 显示提示，并保留受限进程初始化所需的控制台环境。
...
跨平台进程套件钉住普通 Node `windowsHide` 选项、两种受限 stdio 路径的隐藏启动字段、无控制台宿主选择与现有控制台保留。组装后的 runner 行为继续由原生 Windows CI 负责。
