...
生图产物是上游 npm 1.5.1 发行版的仅运行时重打包。它保留未经修改的 Host 与 Client 编译 bundle、441 案例模板快照、包元数据、组合补丁、README 与 Apache-2.0 许可证，同时移除非运行时截图、演示视频、TypeScript 源码与 Source Map。仓库兼容补丁移除插件单独的模型卡片，并为每项任务从**设置 → 模型 → 服务接入**和**使用场景**解析所选生图模型、完整端点、协议与凭据。补丁会分发 OpenAI Images、DashScope 生图与 MiniMax 生图请求格式，并在共享设置变化后刷新 Host 与 Client 指引。生成请求会把提示词与参考图发送给所选外部服务，安装器不包含该服务或模型。技能／MCP 发行产物会应用仓库补丁，把已过时的客户端注入项与会话查询替换为当前 DSH 版本的 renderer 与 session-controller API。Univer 产物从用户提供的 0.2.12 源码重新构建：内嵌开发许可证回退值为空，只有应用环境中的 `UNIVER_LICENSE` 会传给 Viewer、Gateway、渲染进程与 unit-content worker。发行 Univer 配置项设置了 `telemetry: false`。其 Viewer 资源、进程、技能、资源 manifest 与 Windows x64 原生 binding 仍作为普通生产依赖，使桌面载荷门禁能够显式要求这些文件。Univer 封装层采用 Apache-2.0，而其可执行依赖闭包含有三个外部 `@univerjs-pro/*` runtime 根包，以及构建脚本内联的 90 个分别许可的模块，其中 79 个属于 `@univerjs-pro/*`，11 个属于 `@univer-cli/*`。编译后的压缩包既不包含这些模块各自的 manifest，也不包含它们的声明。生成的第三方声明会列出已声明包身份、固定构建期声明摘要，并在摘要变化时要求重新审阅。有效商业许可证与适当的生产和分发权必须覆盖该闭包。执行浏览器渲染的 Slide 操作仍把 Chrome 或 Chromium 作为外部前置条件，并可通过 `UNIVER_RENDER_BROWSER` 指定。

上游插件保留各自的运行时所有权。生图插件持有请求编排、历史、画廊、模板、四项模型工具与 `/edit_image`，共享模型设置则持有所选提供方线路、目录、协议与凭据；IM 继续持有平台连接、QQ 设置、移动通知路由与 `qq_send_local_file`；cron 继续持有通用命令调度、历史、管理界面与四项模型工具；技能／MCP 面板持有技能文件操作、CLI 与所选 profile patch 中的受管 MCP 区块；Univer 持有其十三项工具与审阅生命周期。内置只改变可用性，不改变这些职责。[移动端工作台提醒决策](2026-08-22-mobile-workbench-reminders.zh.md)仍把提醒计时器和确认保存在工作台文档中，而不镜像到 cron。[内置 better-sidebar 决策](2026-08-23-built-in-better-sidebar.zh.md)仍持有工作台挂载与去重防护；本决策同时接受 Office 查看器的 AGPL-3.0 分发义务与独立的 Univer 商业 runtime 义务，并在生成的第三方声明中保留它们。
...
**根据对话模型的图片输入能力推断生图能力。**图片输入描述的是视觉理解，不是生成操作。因此统一注册表使用固定的图像生成模型类型、显式生成协议与完整端点，生图插件继续提供工具、画廊与结果 renderer。
...
- 模型页把全部生图与语音线路放在「服务接入」中，并在「使用场景」中分别提供一次直接分配；生图插件没有单独的配置卡片。连接平台 → QQ 保留机器人与工作区设置，但没有 ASR 表单；QQ 语音附件使用共享语音分配。
...
能力测试覆盖提供方选择与释放、实时媒体模型分配、供应商 profile 与凭据读取、准确的智谱和 Qwen 语音格式、准确的 OpenAI、DashScope 与 MiniMax 生图格式、端点限制、取消、支持的媒体、大小与响应上限、传输与 HTTP 失败分类、JSON 失败及安全诊断。客户端测试覆盖 MediaRecorder 支持与清理、麦克风音量投影、三秒静音收尾、SSR、PCM WAV header、立体声混合、重采样、解码器选择与清理、输入框手势与提交门控、工作台命令、可编辑转写，以及不同的连接失败与请求拒绝通知。模型页浏览器场景会拒绝单独的生图与语音模型卡片、配置供应商线路、在「使用场景」中分配生图与语音模型，并确认凭据不会进入设置文档。插件页浏览器场景确认生图配置不再出现，QQ 工作区场景则确认连接平台 → QQ 中不再存在语音字段。一条无外部依赖的已组装 Web 场景会在输入框与日常管理中录制确定性活动，固定共享启用麦克风状态，等待静音收尾，在真实浏览器 bundle 中转换音频，要求通过生成的 Remote 发出所选 GLM-ASR multipart 请求，并在不调用模型的情况下固定成功转写与模型拒绝提示。IM 桥接测试要求 QQ WAV 下载委托给同一操作，同时保留平台已提供的转写文本。PPT Master 包测试会加载真实提供方，并固定上游版本、许可证摘要、完整性门禁标记、文件数量与逻辑字节数。发行组合场景要求存在四项生图工具、IM 与 cron Host 工具、全部十三项 Univer 工具、六个内置客户端模块，以及可加载的 `ppt-master` 目录条目。桌面载荷测试要求生图 Host 与 Client bundle、模板快照与许可证、技能／MCP 包、PPT Master 完整分发各类资源的代表文件，以及 Univer Viewer、Gateway、worker、技能、商业资源与 Windows x64 原生 binding，之后 Windows workflow 才会把同一生产依赖闭包构建进 NSIS artifact。
