...
输入框与教师工作台控件共用一个 MediaRecorder hook。每次操作都会请求麦克风权限，收集一个完整 WebM、Ogg、MP4、MP3 或 WAV blob，释放全部媒体轨道，把 blob 转为规范 base64，再调用 Host Remote。录制或转写时会禁止冲突的提交操作，接受的文本仍可通过与键入文本相同的草稿、待办、备忘录或账本路径编辑。音频字节和提供方响应不会持久化或写入日志。
...
- 在支持 MediaRecorder 的浏览器中，输入框与日常管理语音输入现在保持一致，但每次请求会在浏览器与 Host 内存中缓冲一整段录音并发送给已配置服务。20 MiB 音频上限与操作截止时间限制了成本；不支持流式或部分转写。
...
能力测试覆盖提供方选择与释放、实时读取 QQ 配置与凭据、端点限制、取消、multipart 字段、支持的媒体、大小与响应上限、HTTP 与 JSON 失败及安全诊断。客户端测试覆盖 MediaRecorder 支持与清理、SSR、输入框手势与提交门控、工作台命令、可编辑转写和失败通知。一条无外部依赖的已组装 Web 场景会在输入框与日常管理中录制确定性音频，通过真实生成 Remote 发送到本地 QQ 兼容 ASR 服务，并在不调用模型的情况下固定最终 UI 快照。发行组合场景要求存在 IM 与 cron Host 工具，以及 Office、IM 与 cron 客户端模块。Windows workflow 把同一生产依赖闭包构建进 NSIS artifact。
