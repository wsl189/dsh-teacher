...
- The compact boundary child spends a fixed output allowance on semantic exceptions. Deployments whose selected model cannot classify a bounded group within that allowance can raise `maxQuestionCompactBoundaryOutputTokens` without changing the progressive fallback.
