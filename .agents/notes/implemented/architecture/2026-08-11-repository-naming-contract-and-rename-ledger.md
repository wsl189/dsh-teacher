...
| `SessionExportDownloadController`, other `SessionExport*` browser types, `useSessionExport`, `SessionExportHeader` | `SessionLogDownloadController`, corresponding `SessionLogDownload*` types, `useSessionLogDownload` | The controller owns preflight, duplicate-request collapse, modal state, and browser save. The default Client mounts only the `/export` result dialog and has no Header action. |
