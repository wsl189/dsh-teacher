...
两次进程启动都请求隐藏窗口：parent 启动 runner 时传入 `windowsHide`，runner 为 target 调用共享 Win32 process owner 时传入 `hideWindow`。仅隐藏其中一个进程，仍可能在没有控制台的桌面宿主中闪现控制台窗口。target 使用 `STARTF_USESHOWWINDOW` 与 `SW_HIDE`，保留 suspended creation、Job assignment、标准句柄继承及退出结果上报。native 测试让隔离宿主脱离控制台，并通过 provider 检查 PowerShell 7 与 Windows PowerShell 的窗口可见性、输入、输出及退出状态。
