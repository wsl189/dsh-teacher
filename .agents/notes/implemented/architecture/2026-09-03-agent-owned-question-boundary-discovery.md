...
Host tests submit three OCR-damaged bracketed heads that are absent from `possibleQuestionHeadIds` and require all three accepted questions. They also require the inline prompt and tool schema to request one complete `questions` draft, preserve explicit decisions for hinted candidates, fall back to source tools when complete inline evidence exceeds its character limit, allow complete-group missing-question findings independent of the hint list, split a missing question out of a combined crop, and keep page-only recovery forbidden during crop-local recuts. The assembled Web question-segmentation test drives the complete-draft schema through the shipped composition.
