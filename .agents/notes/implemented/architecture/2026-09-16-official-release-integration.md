...
The Web profile mounts official Files, Documents, and Terminal sidebar plugins. The retained Office preview plugin registers with the official document-preview service. Its adapter supplies a full-height flex container because the Office viewers depend on parent flex sizing; a block parent can load workbook data while leaving the sheet canvas at zero height. Univer owns its nested React root's DOM removal during disposal; clearing the host separately can race that removal when switching tabs. Browser regressions exercise DOCX, XLSX, and PPTX through Files, require visible sheet canvas content, and cover pane resizing and tab switching. No `dsh-better-sidebar` package, draft-tab adapter, Windows-MCP package, or private Python runtime belongs to the distribution.
