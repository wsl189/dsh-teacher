...
- The directory is compartmented by `SessionId`. Ordinary warm and push-invalidation pulls are single-flight per key, while an epoch guard prevents an explicitly superseded pull from publishing stale state. `commands/change` soft-invalidates ready keys: an initial Agent-mount pull absorbs registry notifications already represented by its result, notifications during a ready-key repull coalesce into at most one follow-up after success, and failure does not retry from another change notification. Explicit warm or Enter demand and hard resets may retry. `connection/reset` hard-invalidates every key and rewarms, Enter strong-waits on the current key, and a failure keeps the draft with no downgrade. Prewarming hangs on the source's `warm` hook — once over the full roster at scope birth, which covers the entire session lifecycle (session capability is constant from birth).
...
| Client packages self-reporting command directories | The host is the single source of truth; the client only reads descriptors, with `commands/change` pushing invalidation |
| Start one directory pull for every `commands/change` frame | Agent activation emits registry changes while a list pull is pending; a failed mount would turn those frames into recursive requests, so pending and failed entries absorb them and only explicit demand retries |
...
- The resident directory cache plus push invalidation buys zero-latency menus and reliable enter adjudication; the cost is soft-change coalescing, hard reconnect and preset resets, and epoch ordering that all need tests pinning them.
