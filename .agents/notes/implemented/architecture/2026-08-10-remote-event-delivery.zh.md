...
两个 chat e2e 继续直接引 `dsh-client-runtime/client`，因为 `runtime` 工程本来就在 Host 图里；其他必需的客户端常量或纯 helper 则在测试侧镜像，不增加工程引用。这样 4 个消费者可以离开 Host 图，`apps/cli/tsconfig.json` 中 15 条 Client 工程引用也不再承担 owner-map 作用。镜像值必须与源逐字一致，使漂移通过选择器失配或过期断言响亮失败。
