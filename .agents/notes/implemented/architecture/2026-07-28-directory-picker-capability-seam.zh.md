...
- **依赖调研（手写 vs 引入）。** Node 标准库本身就是维护中的跨平台 OS 层（`opendir`、`stat`、`homedir`、路径语义）；调研过的替代品都过不了依赖门槛——文件管理器包（`node-file-manager`、`files-and-folders`、Syncfusion 的提供方）是整套 HTTP 应用（契合度不过），盘符工具（原生扩展 `drivelist`、约七年未更的 `windows-drive-letters`）健康度／比例失当。Node 没有盘符枚举 API，因此 browse 后端用 `stat` 并发探测从 `A:\` 到 `Z:\`，略过不可用根目录，并把主目录所在根目录（包括 UNC 主目录共享）保留为已知的列举锚点。结果仍是标准库上的薄适配器。
...
- **整个文件系统均可浏览，按需发现根目录，不做 roots 配置。** `workspace.create` 接受任意路径且 API 本就提供驱动 bash 的方法，因此配置式浏览根只会是 UX 范围而非边界。「连接平台」机器人工作区选择器提供了明确的跨盘消费方：`listRoots(signal)` 现在在每次打开对话框时发现一次跳转目标，并与初始主目录列举相互独立；该选择器与第一方 Miller 浏览器仅在有多个可用根目录时显示磁盘选择器。单个不可用盘符探测会被略过；调用方取消会结束请求，不等待迟到的 `stat` 结算。任意 UNC 共享发现与部署定义的虚拟根目录仍不属于该 seam，直到配置式来源有明确消费方。
...
- **把根目录附在每个 `DirectoryListing` 上。** 否决：这会在每次导航时重复稳定的根目录元数据，并让首个目录视图等待所有 Windows 盘符探测。独立的 `listRoots` 动词让对话框只发现一次根目录，同时让普通主目录列举独立进行。
...
- 协议公开生成的 `directoryPicker/listRoots`、`directoryPicker/list` 与 `directoryPicker/createDirectory` 方法及四个错误码；Connection fixture 提供确定性根目录列表、浏览树与 `directoryPicker/pick` 结果供无密钥组装测试使用。
