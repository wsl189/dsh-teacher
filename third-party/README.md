...
The pinned skills/MCP panel uses a compatibility patch for alpha.2 Typert codec factories; its version, saved skills, groups, and server configuration remain unchanged. Univer has no Client entry, popup, live preview, review page, or plugin settings card. DSH retains the separate Office sidebar renderer. Content workers and background rendering remain available for AI editing, screenshots, and export.

Published npm artifacts retain their licenses and source metadata. `pnpm-workspace.yaml` names each compatibility patch; `pnpm-lock.yaml` fixes the resolved closure. AnySearch remains the reviewed 0.1.4 source build. Univer keeps the pristine npm archive beside `dsh-univer-office-0.3.0-dsh.8.tgz`, [runtime.patch](dsh-univer-office/runtime.patch), and [headless.patch](dsh-univer-office/headless.patch). Run `node third-party/dsh-univer-office/repack.mjs` with GNU tar, patch, and gzip to reproduce it. The repack removes `lib/client.js` and `artifacts/viewer`; the Gateway serves content APIs and a JSON health endpoint. Before Office publication or cleanup, it closes only idle cached databases inside the verified temporary directory; active operations block cleanup and preserve the source.
