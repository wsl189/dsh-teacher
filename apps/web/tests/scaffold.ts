...
export interface LaunchOptions {
  ...
  /** Deterministic MinerU endpoint for document extraction. */
  ocrEndpoint?: string
  /** Deterministic speech endpoint for the selected model route. */
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  try {
    ...
    if (ctx.settings.describe().some(section => section.ns === WELCOME_NOTICE_SETTINGS_NAMESPACE)
      && options.welcomeNoticePending !== true) {
