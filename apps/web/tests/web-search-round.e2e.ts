...
// drives `web_search`; the model stream is replayed while the real AnySearch
// provider calls a deterministic local endpoint without an API key.
 ...
 * scroll container are both exercised. Titles and snippets make the eight
 * kept rows exceed the `.sources` 320px max-height.
 */
...
/** Start the deterministic AnySearch API double used by the real provider. */
    ...
    request.on('end', () => {
      ...
      response.end(JSON.stringify({
        ...
        data: {
          ...
          metadata: { total_results: PROVIDER_RESULT_COUNT, search_time_ms: 5 },
        },
      }))
    })
  })
...
describe('web e2e: shipped default web search', () => {
  ...
  it.skipIf(MODE === 'record')('uses the real provider and persists the capped structured result', () => {
    ...
    for (const query of QUERIES) {
      ...
      expect(request).toMatchObject({ path: '/v1/search', authorization: undefined })
    ...
    expect(auxiliaryRequests).toHaveLength(0)
