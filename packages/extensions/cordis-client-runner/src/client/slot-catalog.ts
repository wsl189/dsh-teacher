...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'Additive root-scoped seat layered inside the center column, above the mounted conversation.',
    doc: 'Additive root-scoped seat layered inside the center column, above the\nmounted conversation. Entries render nothing while inactive and fill\nthe seat when presenting a primary in-app surface that must preserve\nthe navigation and details columns.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'shell.main\', () => ctx.slots.register(\n      { name: \'shell.main\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'Product workbench entry between New Session and workspace browsing.',
    doc: 'Product workbench entry between New Session and workspace browsing.\nThe occupant owns its disclosure and feature navigation; the shell\nsupplies only column state and a rail-to-wide expansion request.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-teacher-workbench SidebarWorkbench',
    ],
    replaceRisk: 'shadows-shipped-ui',
