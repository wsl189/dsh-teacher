...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, model-retry, steering, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'session',
    summary: 'Body renderer for a produced file selected in the shared details store.',
    doc: 'Body renderer for a produced file selected in the shared details store.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      'client-ui-deliverables FilePreview',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-teacher-workbench TeacherWorkbenchSettingsRow id \'teacher-workbench\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-plugins MinerUCard',
      'client-ui-settings-plugins TeacherWorkbenchCard',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-teacher-workbench WorkbenchSurface id \'teacher-workbench\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'sidebar.primary.section',
    kind: 'list',
    scope: 'root',
    summary: 'Optional primary sections between the brand row and the workspace browser.',
    doc: 'Optional primary sections between the brand row and the workspace browser.\nEntries receive the column state and may request expansion from the rail.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-teacher-workbench SidebarWorkbench id \'teacher-workbench\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.primary.section\', () => ctx.slots.register(\n      { name: \'sidebar.primary.section\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.chat.toolGroup\' (client-ui-tool), so it exists while that entry is mounted',
    occupants: [
