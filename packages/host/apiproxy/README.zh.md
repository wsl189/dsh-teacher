...
普通 `session.prompt` 还可以携带受限的 OCR 文档上下文。Host 会在普通人类消息之前逐条注入，把它们记录成 `source.kind = plugin`、`plugin = mineru-ocr` 且以一次性 `notice` 摘要点名文件的用户角色消息；模型可见文本包含文件名、截断状态与提取 Markdown。源文件字节不进入 Session 日志。上下文顺序与请求一致，排队和插话发送都会把人类消息放在这些上下文之后。子智能体续聊协议只接受文本，因此拒绝该字段。
...
`session.previewFile` 是供产品内渲染器使用的纯数据对应接口。请求携带会话 id，以及 workspace 相对路径或已被约束的绝对路径；Host 从该会话派生 workspace 根目录，通过 `ctx.fs` 解析两端，并拒绝路径穿越以及规范目标离开 workspace 的符号链接。成功响应以 Base64 返回完整文件及其字节数。`previewFileMaxBytes` 是缓冲前施加的含端点上限，默认 40 MiB；设为零会禁用该方法。文件缺失、不是普通文件、不可读或过大时，会以带稳定原因的 `file-preview-unavailable` 返回。请求信号可取消文件系统工作，MIME 类型与渲染器选择都不会经过该 RPC。
