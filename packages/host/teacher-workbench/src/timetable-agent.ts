...
function sourceTool(name: string, regions: readonly SourceRegion[]) {
  return defineTool({
    name,
    ...
    execute(args) {
      if (args.mode === 'inspect') return Promise.resolve(JSON.stringify({ regions: regions.map((region, index) => ({
        region: index, label: region.label, pages: region.pages.length,
      })) }))
      ...
      return Promise.resolve(content === undefined ? 'REJECTED: read needs a valid region and page from inspect.'
        : `${JSON.stringify({ region: args.region, page: args.page, pages: region?.pages.length })}\n\n${content}`)
    },
  })
}
...
function draftTool(
  ...
  accepted: Map<ValidationToken, readonly TeacherTimetableNormalizedEntry[]>,
) {
  const batches = new Map<BatchId, readonly TeacherTimetableNormalizedEntry[]>()
  return defineTool({
    name,
    description: 'Submit timetable entries in manageable batches. common supplies shared fields; each item overrides them. A returned batchId can replace that whole batch for corrections. Rejected batches leave accepted data intact. Finish only after auditing all relevant source pages; expectedTotal must match the complete source schedule. The result is a review draft, never a write to the workbench.',
    parameters: {
      action: { type: 'string', enum: ['submit', 'finish'], required: true },
...
export async function normalizeTimetableWithAgent(
  ...
  try {
    ...
    const accepted = new Map<ValidationToken, readonly TeacherTimetableNormalizedEntry[]>()
    disposers.push(ctx.effect(() => tools.register(sourceTool(sourceName, regions)), 'teacher-workbench: timetable source'))
    disposers.push(ctx.effect(() => tools.register(draftTool(draftName, request, config, accepted)), 'teacher-workbench: timetable draft'))
    ...
    run = await subagents.start('spawn', {
      ...
      outputSchema: {
        type: 'object', properties: { validationToken: { type: 'string', description: 'Token returned by this run after finish accepts the complete draft.' } },
    ...
    const items = parsed.success ? accepted.get(parsed.data.validationToken as ValidationToken) : undefined
    outcome = result.stopReason !== 'completed'
      ? rejected('model-failed', result.diagnostic ?? `The tool model stopped with ${result.stopReason}.`)
      : items === undefined ? rejected('invalid-output', 'The child did not finish a validated timetable draft.')
        : { ok: true, value: { items } }
  } catch (error) {
