...
所有语义页组合并后，`segmentQuestions` 会计算全部已接受且源自 MinerU 的试题区域中的最大归一化宽度。浏览器与普通对话渲染器会保留每个区域自己的左、上、下坐标，再把右边界设为左边界加该最大宽度，并在页面右侧截断。渲染器既不缩放来源像素，也不增加横向补白，因此不同栏目的试题不会合并，而相同尺寸页面通常会按同一栅格倍率生成等宽题图。
