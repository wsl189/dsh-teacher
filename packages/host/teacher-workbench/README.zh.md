...
所有语义页组合并后，`segmentQuestions` 会根据源 PDF 中所有已接受且源自 MinerU 的试题区域，取最小左坐标与最大右坐标，得到整份 PDF 共用的归一化横向裁切边界。浏览器与普通对话渲染器会在这对横坐标之间直接裁取原页面，同时保留每道题自己的上下坐标；既不缩放来源像素，也不增加横向补白，因此相同尺寸页面会按同一栅格倍率生成等宽题图。
...
试题存储字段可在 **设置 → 插件 → 插件配置 → 试题工作区存储**中通过 `teacher-workbench` 设置命名空间修改。Web 组合包默认把媒体、源文档与生成文档根目录设在 `~/.dsh/teacher-workbench/` 下。试题工作区会递归投影当前 `segmentsRoot` 下的每个非隐藏目录，包括空目录和只有图片的叶子目录；直接图片会作为试卷行显示在其所属文件夹内。它还会识别 `studentsRoot` 下的“年份／班级／学生”和“年份／年级／班级／学生”两种层级；匹配的目录会与名册记录合并，每名学生下的全部非隐藏子目录都会成为目录树节点，其中的图片会被递归读取。学生行汇总完整子树中的图片，子目录行只选择该目录。修改任一根目录都会切换界面显示的集合，不会从原根目录复制文件。通过文件系统发现的班级、试卷、图片与名册编辑仍保持只读；目录新建、重命名和递归删除是范围明确的例外：浏览器提交最近一次扫描保留的不透明标识，Host 重新校验目标位于当前根目录内，然后修改该物理目录。重命名持久化学生层级时还会提交显示名称和调整后的分发路径；文档写入失败则回滚物理移动。
