...
所有语义页组合并后，`segmentQuestions` 会计算全部已接受且源自 MinerU 的试题区域中的最大归一化宽度。每个区域还会记录由纵向重叠且不属于该题的右侧内容形成的最近右界。浏览器与普通对话渲染器保留区域自己的左、上、下坐标，最多采样一份最大宽度，同时不越过该右界。安全来源像素会原尺寸放在最大宽度的白色画布上，因此不同栏目不会相互串入，而相同尺寸页面仍会按同一栅格倍率生成等宽题图。
