...
The internal scheduler uses a process-wide symbol key so source and built module imports address the same scheduler on `ctx.tools`. Tool registrations and execution state remain owned by each service instance; the key is not a plugin extension point.
