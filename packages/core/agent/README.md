...
`AgentOptions` supplies the initial provider/model route, an optional adapter-owned reasoning effort, and an optional positive `maxTokens` output cap. The concrete loop validates exact-model call settings, records the effective values in the request header, and applies them to each conversation-model request; an explicit Agent option wins, while omission leaves the adapter or provider route default in control.
