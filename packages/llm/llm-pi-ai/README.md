...
The selected catalog model receives `GenerateOptions.system`, history, tools, `toolChoice`, and sampling fields supported by pi-ai's common streaming API. A required choice maps to the protocol's strongest available tool-use value: `required`, `any`, or Z.ai's `auto`; supplying any choice without tools fails before provider I/O. A hand-declared Z.ai-compatible completion route exposes `off` as a selectable reasoning effort and serializes it as `thinking.type: disabled`, even when its model entry has no reasoning metadata. On the exact `ollama` completion route, a selected Off for a recognized toggleable family serializes as `reasoning_effort: none`; GPT-OSS is never advertised as disableable because Ollama does not support that state. Each retained image is preceded by text naming its complete attachment id and actual request dimensions. When the current execution filesystem maps the attachment provider's host object, the text also carries a read-only normalized-object path and warns that normalization or request projection may have resized or re-encoded the upload. When accumulated base64 image payload exceeds the route's `maxRequestImageBytes`, each offloaded image keeps its own identity and currently resolved access in replacement text. Offloaded normalized attachments are not read or transformed. Provider-native replay metadata is restored only when the adapter validates it for the historical content.
