  ...
  private async * streamWithSnapshot(
    ...
    try {
      ...
      const events = snapshot.models.streamSimple(requestModel, context, {
        ...
        // The per-API pi-ai adapters accept required/any; its generic simple type only lists auto/none.
        ...toolChoice === undefined ? {} : { toolChoice: toolChoice as SimpleStreamOptions['toolChoice'] & string },
      ...
      const iterator = toStreamChunks(events, requestModel.contextWindow, options.signal, model.id)[Symbol.asyncIterator]()
      let exhausted = false
      try {
        while (true) {
