...
- **Console creation flags are unavailable** — children created with `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` die during DLL initialization with `STATUS_DLL_INIT_FAILED` (`0xC0000142`). A child shares an existing host console; when a GUI host has no console window, the backend requests a hidden initial window through `STARTF_USESHOWWINDOW` / `SW_HIDE` instead. Pipe-based stdio redirection is unaffected.
