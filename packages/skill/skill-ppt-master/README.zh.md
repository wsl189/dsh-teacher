...
为会话加载时，skill 会包含当前工作区，并使用 [Office 工作区工具](../../deliverables/office-workspace/README.zh.md)管理项目初始化、脚本、资源、导出和验证。`project_manager.py init --dir` 选择分配的目录。检查后的最终文件先发布，再移除临时项目和备份。

分配的目录位于会话工作区内，供各个沙箱命令和宿主工具共享。如果工具不可用，指令要求使用唯一的工作区子目录，并在复制最终文件后显式清理。用户原件保留在临时存储之外。
...
提供方根据每次查询的 `cwd` 追加项目位置指令，不修改上游文件及其归属检查。不带 `cwd` 的查询只返回上游正文。资源路径仍指向已安装的 skill 目录。
...
目录摘要会改变会话前缀中的 skill 列表。加载 `ppt-master` 会在工具结果插入点加入其路由入口指令和可用的会话工作区；后续引用文件只影响读取它们的轮次。
