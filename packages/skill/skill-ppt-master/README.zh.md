...
为会话加载时，skill 会包含当前工作区，并要求初始化项目时通过 `--dir` 指定该工作区或其子目录。用户明确请求其他项目或输出位置时，仍须遵守会话权限。除非用户请求其他输出路径，skill 会保留导出器默认的项目内输出和备份行为。

Office 生成指引还要求把可复用脚本、虚拟环境以及跨调用或跨工具使用的文件放在会话工作区，因为 `/tmp` 等临时目录可能彼此隔离。该指引不会改变沙箱权限或临时目录行为。
