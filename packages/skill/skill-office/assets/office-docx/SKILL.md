...
Keep shared runtime and skill resources read-only. For generation, call `office_workspace` with `action: "create"` when available and put all scripts, images, renders, reports, and draft outputs in its returned directory. If unavailable, allocate a unique temporary subdirectory in the task workspace. Keep user originals outside it; copy inputs when needed. Use the requested language and preserve the supplied file's design unless a redesign is requested.
...
After all required checks, wait for authoring and rendering processes to exit. Call `office_workspace` with `action: "finish"`, its `directory`, and `files: [{ source, destination }]` to publish only requested final files to new workspace paths and remove the intermediates. Present those returned paths. If the tool is unavailable, copy the checked final files out before deleting only the temporary directory you allocated. Do not deliver scripts, reports, renders, backups, or source projects unless requested. When waiting for user choices across turns, call `pause` before the wait and `resume` before continuing. Paused projects survive a normal waiting turn. Final deliverables must live in the session workspace, outside temporary directories. Presenting a managed temporary file moves it to the workspace; use the returned path. At turn end, cancellation, or errors, remaining DOCX, XLSX, and PPTX files are recovered before cleanup. Use `finish` to select the checked finals or `discard` to abandon the draft.

Call `present({"files":[{"path":"report.docx"}]})` with the actual final DOCX path. Keep the delivered workspace file in place and do not present temporary QA reports unless requested. If `present` is unavailable, provide the final workspace path using the session's supported file delivery method.
