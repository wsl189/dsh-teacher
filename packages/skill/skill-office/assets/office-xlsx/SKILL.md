...
After all required checks, wait for authoring and rendering processes to exit. Call `office_workspace` with `action: "finish"`, its `directory`, and `files: [{ source, destination }]` to publish only requested final files to new workspace paths and remove the intermediates. Present those returned paths. If the tool is unavailable, copy the checked final files out before deleting only the temporary directory you allocated. Do not deliver scripts, reports, renders, backups, or source projects unless requested. When waiting for user choices across turns, call `pause` before the wait and `resume` before continuing. Paused projects survive a normal waiting turn. Other tool-managed temporary directories expire when the current turn ends, including cancellation and errors; finish before ending the turn or call `discard` to abandon the draft.
