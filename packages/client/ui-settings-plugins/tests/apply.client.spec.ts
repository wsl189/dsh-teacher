...
describe('ui-settings-plugins apply', () => {
  ...
  it('registers the Plugins settings section with the workbench configuration tab', async () => {
    ...
    expect(slots.entries('settings.plugin.item').map(entry => entry.options.key)).toEqual(['ocr-mineru', 'teacher-workbench'])
  })

  it('injects a live tab projection and one business face per configuration page', async () => {
    const { ctx, slots } = await bench(['shell', 'agent-loop', 'subagent', 'subagent-model-selection', 'web-search-anysearch'])
    declareRoot(slots)
    await ctx.plugin({ inject: [...inject], apply }).await()
    ...
    expect(initialTabs).toEqual([{ id: 'configurable', order: 0, label: '插件配置' }])
  ...
  it('registers one configuration page per served namespace, in its own order, titled in the active locale', async () => {
    const { ctx, slots } = await bench(['web-search-anysearch', 'shell', 'agent-loop', 'subagent-model-selection'])
    declareRoot(slots)
  ...
  it('registers the pages of the served namespaces only, and withdraws one the Host stops serving', async () => {
    ...
    const { ctx, slots, describeSettings, remote } = await bench(['agent-loop', 'ui-theme', 'web-search-anysearch'])
    declareRoot(slots)
    await ctx.plugin({ inject: [...inject], apply }).await()
