...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。六张内置卡片按命名空间是否可用依次显示：shell 执行器（`bash`）、agent loop 的工具调用并行度（`agent-loop`）、subagent 模型选择（`subagent-model-selection`）、AnySearch 网页搜索（`web-search-anysearch`）、MinerU 服务（`ocr-mineru`）和文档提取（`teacher-workbench`）。
