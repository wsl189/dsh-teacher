...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。对应命名空间被服务时，六张内置卡片依次为 shell 执行器（`bash`）、agent 循环工具调用并行度（`agent-loop`）、subagent 模型选择（`subagent-model-selection`）、AnySearch 网页搜索（`web-search-anysearch`）、MinerU 服务（`ocr-mineru`）以及文档提取（`teacher-workbench`）。
...
「文档提取」卡片包含默认关闭的切题思考开关。开启后，题界识别、视觉复核与修复使用已启用的推理档；关闭后优先选择模型公布的 Off 档，否则使用最低公布档位或提供方默认值。
...
「网页搜索」卡片绑定内置 AnySearch 命名空间。密钥控件写入分节所指定的引用（默认为 `ANYSEARCH_API_KEY`），服务地址与单次搜索结果上限变更则存入 settings 文档。结果上限接受 1–20，默认为 8，且高级工具调用请求的更小值仍然生效。三类变更都会作用于下一次操作；未配置密钥时继续使用匿名访问。
