...
Open the Plugins section in Settings and select the **Plugin configuration** tab to edit the host-plane plugins this deployment composes. The six built-in cards appear in this order when their namespaces are served: the shell executor (`bash`), agent-loop tool-call parallelism (`agent-loop`), subagent model selection (`subagent-model-selection`), AnySearch web search (`web-search-anysearch`), the MinerU service (`ocr-mineru`), and document extraction (`teacher-workbench`).
...
The Document extraction card includes the default-off question-cutting reasoning switch. Enabling it applies an enabled reasoning effort to boundary detection, visual review, and repair; disabling it selects Off when advertised, otherwise the model's lowest advertised effort or provider default.
