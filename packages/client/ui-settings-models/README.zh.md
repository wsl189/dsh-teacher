...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限和输入类型收在该行的展开区内，右侧是两个无文字的操作——展开与删除。输入类型是显式值：「仅文本」存储 `[text]`，「文本和图片」存储 `[text, image]`，因此模型级选择会覆盖路由回退值。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
...
通过产品持有的后台 agent 所使用的工具模型选择间接影响；该浏览器包本身不组装或发送模型请求。
