...
`useVoiceRecorder` 为所有浏览器语音控件持有麦克风获取、MediaRecorder 释放、完整音频准备、转写状态，以及规范化结果或失败回调。`normalizeSpeechRecording` 会原样保留 WAV；其他格式则使用宿主的标准或 WebKit AudioContext 解码器，把完整录音混合并重采样为 16 kHz 单声道 PCM WAV。没有 Web Audio 解码能力的宿主会保留 MediaRecorder 容器以维持兼容。转换与转写共用同一个等待状态，所有媒体轨道会在转换开始前停止，本包不会持久化原始或准备后的音频。
