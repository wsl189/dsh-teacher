...
本包让生成的图片持续显示在 dsh Web 对话的最终回答旁。它把内置生图工具中经过验证的图片引用按 Turn 汇总为一个独立 Chat 节点。该节点跟随 Assistant 回答并位于紧凑过程折叠之外，现有 Tool 卡片则继续向图片工作室输送数据。它通过提供方仅限 loopback 的附件路由读取图片字节，并在读取失败时提供重试。
...
每张卡片都经 `/api/dsh-imagegen/agent-image` 加载不可变图片，可在新标签页打开原图，并在卡片离开页面时撤销浏览器 object URL。失败的请求会变为本地化重试按钮。同一 Turn 中的其他生成图片仍会独立加载。
...
keyed `image-generation-result` renderer 使用现有 `conversation` 图片文案和提供方的 Host 路由。本包不会在运行时导入另一个 Client 功能、修改 Session 事件、授权通用附件读取、抑制 Tool 视图或分发图片工作室状态。
...
- **最终卡片是预览，不是图片工作室**：复制、下载、编辑、图库与任务控制仍位于提供方拥有的 Tool 卡片和工作室中；因此展开过程时可能同时看到两处预览。
