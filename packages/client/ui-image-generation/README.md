...
This package keeps generated images visible beside the final answer in the dsh Web conversation. It groups validated image references from the bundled image-generation tools into one independent Chat node per Turn. The node follows the Assistant answer and stays outside compact process folding, while the existing Tool card continues to feed the image studio. It reads image bytes through the provider's loopback-only attachment route and offers a retry when that read fails.
...
Each card loads the immutable image through `/api/dsh-imagegen/agent-image`, opens the original in a new tab, and revokes its browser object URL when the card leaves the page. A failed request becomes a localized retry button. Other generated images in the same Turn continue to load independently.
...
The keyed `image-generation-result` renderer uses the existing `conversation` image strings and the provider's Host route. The package does not import another Client feature at runtime, modify Session events, authorize generic attachment reads, suppress the Tool view, or dispatch image-studio state.
...
- **The final card is a preview, not the image studio** — copy, download, editing, gallery, and task controls remain in the provider-owned Tool card and studio; an expanded process can therefore show both previews.
