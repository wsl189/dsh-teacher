...
Document uploads extract in the background and never copy OCR Markdown into the visible draft. Ready rows accompany the next send as ordered hidden prompt contexts, while extracting or failed rows block submission until they settle or are removed; the browser retains each source file for draft preview and workbench reopening until the row is admitted, removed, or disposed. The microphone uses the configured Host speech Remote and inserts the accepted transcript at the live Lexical selection. Its neutral microphone glyph pulses while listening and fills upward with the sampled level; three seconds without audible input finishes the recording and starts transcription when Web Audio metering is available. A short Space press remains an ordinary space edit; holding Space for 500 milliseconds starts the same recorder, and key release remains an early manual stop.
