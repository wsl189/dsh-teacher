...
The composer also retains OCR document uploads and shared voice transcription. Extracted documents remain separate from the visible draft and enter the logged request context on submission. OCR failures block sending until the file is removed or successfully extracted. Voice input uses the speech model assigned under Settings → Models; workspace switches carry unsent documents with the draft.
