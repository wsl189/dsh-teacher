...
export class InputHub implements SessionInputResolver {
  ...
  shellFor(binding: SessionBinding): SessionInputShell {
    ...
    const shell = new SessionInputShell({
      ...
      hasDefaultPayload: () => this.conversation().hasDraftDocuments(session.sessionId),
  ...
  private sink(
    ...
    if (text === '' && attachmentIds.length === 0 && !this.conversation().hasDraftDocuments(session.sessionId)) return Promise.resolve({ kind: 'success' })
