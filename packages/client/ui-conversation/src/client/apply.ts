...
import type { DraftDocument } from './service.ts'
...
export const inject = [
  'remote', 'remote.ocr', 'remote.speech', 'remote.teacherWorkbench',
...
const ABSENT_DOCUMENTS = { getSnapshot: () => EMPTY_DOCUMENTS, subscribe: () => () => {} }
...
export function apply(ctx: Context, config: Config = Config({})): void {
      ...
      selectWorkspace: workspaceId => workspaceNavigation.openWorkspace(workspaceId, (nextId) => {
        if (sessionId !== undefined && nextId !== sessionId) {
          ...
          if (attachmentIds.length === 0 || next.addAttachments(attachmentIds)) {
            ...
            concreteConversation(ctx).moveDraftDocuments(sessionId, nextId)
            if (draft !== '') {
              next.setDraft(draft)
              from.setDraft('')
            }
  ...
  ctx.plugin(ConversationController, {
    ...
    ocr: ctx.remote.ocr,
    workbench: ctx.remote.teacherWorkbench,
