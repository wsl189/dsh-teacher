...
describe('image draft rail', () => {
  ...
  it('keeps OCR text outside the editor and gates send until document extraction completes', () => {
    ...
    expect(result.shell.snapshot.draft).toBe('导入学生名册')
    expect(result.view.getByText('MinerU 识别中…')).toBeTruthy()
    expect(result.button.disabled).toBe(true)
    const owner = attachmentOwner(result.slotCalls)
    expect(owner.documents).toEqual([pending])
    expect(owner.resolveDocumentFile(pending.id)?.name).toBe('roster.xlsx')
    ...
    act(() => { result.documents.set([{ ...pending, status: 'ready' }]) })
  ...
  it('keeps send disabled after OCR failure so the document must be removed or retried', () => {
    const result = bench({
      draft: '分析成绩',
    ...
    expect(result.button.disabled).toBe(true)
    expect(result.view.getByText('scores.pdf').closest('[data-document-status]')?.getAttribute('title'))
      .toBe('MinerU offline')
  })

  it('uses one recorder for the microphone button, tapped Space, and held Space', async () => {
    ...
    try {
      ...
      act(() => {
        keyboard.shell.editor.update(() => { $selectDetectSpan({ start: 1, end: 1 }) }, { discrete: true })
      })
      fireEvent.keyDown(keyboard.textarea, { key: ' ' })
      act(() => { vi.advanceTimersByTime(200) })
      fireEvent.keyUp(keyboard.textarea, { key: ' ' })
      expect(keyboard.shell.snapshot.draft).toBe('A ')
