...
type ClassDraft = Pick<TeacherClass, 'name' | 'grade' | 'subject'>
    ...
    <section className={css.module} aria-label={props.t('module.timetable')}>
      <div className={css.moduleToolbar}>
        ...
        <div className={css.toolbarActions}>
          <TimetableFilters
            ...
            onAddClass={() => {
              setClassDraft({ name: '', grade: activeGrade, subject: props.settings.defaultSubject })
            }}
            t={props.t}
          />
          {view !== 'today' && (
            <>
              ...
              <div className={css.timetableCourseActions}>
                ...
                <button type="button" className={css.buttonPrimary} onClick={() => { openEditor() }}>
                  <Plus size={16} />
                  {props.t('timetable.add')}
                </button>
              </div>
            </>
          )}
      ...
      {classDraft !== null && (
        <EditorModal
          ...
          onSave={() => {
            void props.commands.saveClass({ ...classDraft, usage }).then((result) => {
        ...
        >
          <>
            ...
            <FormField label={props.t('class.grade')}>
              <input value={classDraft.grade} onChange={(event) => { setClassDraft({ ...classDraft, grade: event.target.value }) }} />
            </FormField>
            <FormField label={props.t('class.subject')}>
              <input value={classDraft.subject} onChange={(event) => { setClassDraft({ ...classDraft, subject: event.target.value }) }} />
            </FormField>
          </>
        </EditorModal>
      )}
...
function TimetableFilters(props: {
  ...
  const addClass = (
    <button type="button" className={css.buttonSecondary} onClick={props.onAddClass}>
      <Plus size={16} />
      {props.t('timetable.addClass')}
    </button>
  )
  if (props.view === 'grade') {
    return (
        ...
        <tbody>
          {props.classes.flatMap(owner => periods.map((period, periodIndex) => (
            <tr key={`${owner.id}-${String(period)}`}>
              {periodIndex === 0 && <th rowSpan={periods.length}>{owner.name}</th>}
              <th>{props.t('timetable.periodValue', { period })}</th>
              {WEEKDAYS.map(day => (
                <td key={day}>
                  {props.entries
                    .filter(entry => entry.classId === owner.id && entry.period === period && entry.weekday === day)
                    .map(entry => (
                      <TimetableEntryCard
                        key={entry.id}
                        entry={entry}
        ...
        <tbody>
          {props.rowLabels.map((label, index) => (
            <tr key={`${label}-${String(index)}`}>
              ...
              {WEEKDAYS.map(day => (
                <td key={day}>
                  {props.rowEntries[index]?.filter(entry => entry.weekday === day).map(entry => (
                    <TimetableEntryCard
                      key={entry.id}
                      entry={entry}
