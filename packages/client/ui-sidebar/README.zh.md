...
`SidebarRootComponentProps` 组合布局 owner share、已声明的 `sidebar.workbench`、`sidebar.workspaces`、`sidebar.settings` 与 `sidebar.footer.action` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
