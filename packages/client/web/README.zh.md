...
`PLATFORM_MODULES`（位于 `src/platform.ts`）列出外壳预置的共享模块——React、Cordis、零依赖的 `@deepseek-ai/dsh-concurrency` 工具与静态 UI 库——并与 `PRELOADED_CLIENT_EXTERNALS`（parser 预载的运行时行）一起定义每个动态 bundle 解析所依据的隐式 external 基座。Teacher Workbench 等浏览器插件导入平台预置的静态工具时，仅将其声明为开发依赖，不在 `dsh.client.external` 中重复列出。该字段只添加基座之外的精确请求；参见[共享模块与模块图](../AGENTS.md#shared-modules-and-the-module-graph)。
