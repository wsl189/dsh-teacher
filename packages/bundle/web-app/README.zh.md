...
**设置 → 模型**保留供应商服务接入，以及对话、工具、生图和语音的使用场景分配。已保存的端点、协议、模型、凭据和选择保持原值。详见[模型设置参考](../../client/ui-settings-models/README.zh.md)。
...
Windows profile 加载官方[原生 Cua Driver 提供方](../../experimental/computer-use-cua-driver-native/README.zh.md)。npm SDK 提供平台原生二进制文件，桌面权限由启动应用管理。发行版不包含 Windows-MCP 或其私有 Python 运行时。
