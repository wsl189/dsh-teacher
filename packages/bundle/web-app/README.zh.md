...
浏览器名录始终包含 [ui-desktop-update](../../client/ui-desktop-update/README.zh.md)。普通浏览器中的该配置项不会注册侧边栏占位者；只有已安装 Electron preload 明确提供能力后，更新操作才会出现。

## 内置扩展与语音输入

发行版 Host 与客户端名录直接从本组合包的依赖闭包挂载 `@xmanrui/dsh-im` 1.0.3、`dsh-plugin-cron` 0.1.3 和采用 AGPL-3.0 的 `@huanlin/dsh-plugin-better-sidebar-plugin-office` 0.1.2。因此源码检出或 Windows 安装版都不需要再向每个 profile 单独安装插件。可执行代码随应用发布；机器人凭据、cron 记录、QQ 配置与用户数据仍保存在 `DSH_HOME` 下，需要另行迁移。

本组合包还挂载 [`dsh-speech`](../../speech/speech/README.zh.md) 及其 [`qq-config` 提供方](../../speech/speech-qq/README.zh.md)。提供方会为每次完整录音重新读取 dsh-im 的 QQ 语音文档与凭据，因此对话和教师工作台麦克风控件直接跟随 QQ 设置页，不需要第二套语音配置命名空间，也不需要重启 Host。ASR 服务器与模型仍是外部部署服务。

## 内置侧边栏工作台

Web 组合包把 `dsh-better-sidebar` 声明为运行时依赖，并通过 `web-better-sidebar` 配置项挂载。工作台默认收起，为每个标准 Web profile 提供右侧文件资源管理器、编辑器、终端、Git、浏览器与扩展注册表。如果 profile 仍在 `dsh-web-app` 之后列出独立的 `dsh-better-sidebar` 组合包，该配置仍然有效：独立组合包会检测前面的内置配置项，并禁用多余挂载。

工作区资源管理器无需其他包即可预览 Markdown、图片和 PDF。从工作区打开 Word、Excel 与 PowerPoint 文件需要外部扩展，因为该包使用 AGPL-3.0，不能进入宽松许可证的运行时依赖集合。通过输入框上传且仍保留在浏览器中的文件改用 `ui-attachment` 的临时预览标签页，不需要 profile 扩展。工作区查看器的安装命令、支持的扩展名与重启要求见 [Web UI 指南](../../../docs/user/guide/index.zh.md#preview-workspace-files)。
