...
The browser roster always includes [ui-desktop-update](../../client/ui-desktop-update/README.md). That row registers no sidebar occupant in an ordinary browser; the installed Electron preload is the explicit capability that makes the update action appear.

## Built-in extensions and voice input

The shipped Host and client rosters mount `@xmanrui/dsh-im` 1.0.3, `dsh-plugin-cron` 0.1.3, and the AGPL-3.0 `@huanlin/dsh-plugin-better-sidebar-plugin-office` 0.1.2 directly from this bundle's dependency closure. A source checkout or Windows installer therefore needs no per-profile plugin installation. Executable code ships with the app, while bot credentials, cron records, QQ configuration, and user data remain under `DSH_HOME` and migrate separately.

The bundle also mounts [`dsh-speech`](../../speech/speech/README.md) with the [`qq-config` provider](../../speech/speech-qq/README.md). The provider reads dsh-im's QQ speech document and credential for every completed recording. Conversation and Teacher Workbench microphone controls therefore follow the QQ settings page without a second speech configuration namespace or a Host restart. The ASR server and model remain external deployment services.

## Built-in sidebar workbench

The Web bundle declares `dsh-better-sidebar` as a runtime dependency and mounts it under the `web-better-sidebar` row. The workbench starts closed and adds the right-side file explorer, editor, terminal, Git, browser, and extension registry to every standard Web profile. A profile that still lists the standalone `dsh-better-sidebar` bundle after `dsh-web-app` remains valid: the standalone bundle detects the earlier built-in row and disables its redundant mount.

The workspace explorer previews Markdown, images, and PDF without another package. Word, Excel, and PowerPoint files opened from the workspace require an external extension because its package is AGPL-3.0 and cannot enter the permissively licensed runtime dependency set. Browser-held files uploaded through the composer use `ui-attachment`'s transient preview tab instead and need no profile extension. See the [Web UI guide](../../../docs/user/guide/index.md#preview-workspace-files) for the workspace-viewer installation command, supported extensions, and restart requirement.
