...
The Web profile and Windows desktop include `@anysearch/anysearch-dsh` 0.1.4. Standard `web_search` and `web_fetch` use AnySearch; `anysearch_capabilities`, `anysearch_search`, and `anysearch_batch_search` add capability discovery, vertical search, and batches of up to five requests. The Web bundle disables the inherited `web-search-deepseek` row, so Web and desktop do not load the DeepSeek search provider or its settings section. No separate plugin installation is needed. In the GUI, **Settings → Plugins → Plugin configuration → Web search** stores the optional key, endpoint, and per-search result cap, and each change reaches the next operation. The headless and SDK profiles keep their existing defaults.

Without `ANYSEARCH_API_KEY`, the plugin sends anonymous requests, subject to the service's quota and rate limits. An optional key is resolved through DSH credentials on every operation, including inherited environment values; neither the key nor an account is embedded or provisioned by the installer. Queries and extraction URLs are sent to `https://api.anysearch.com`, so an internet connection is required. Authentication and quota failures are reported without automatically switching providers or retrying an invalid key anonymously. The `web-search-anysearch` row accepts `apiKeyEnv`, `baseURL`, `maxResults`, and `maxRenderedContentChars`; `maxResults` defaults to 8, accepts 1 through 20, and caps every standard, advanced, and batch search while preserving a smaller request limit. The settings card exposes the key reference, endpoint, and result cap, while the rendered-content cap remains composition-owned. The `web` row selects the search and fetch providers.
