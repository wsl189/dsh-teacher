...
export class SessionCommandController {
  ...
  async prompt(request: SessionPromptRequest): Promise<SessionPromptValue> {
    validateDocumentContexts(request.contexts)
    ...
    const admit = async (): Promise<SessionPromptValue> => {
      try {
        ...
        for (const document of request.contexts ?? []) {
          const workbenchSource = document.sourceId === undefined
            ? ''
          ...
          const framing = `Uploaded document ${JSON.stringify(document.name)}${document.truncated ? ' (truncated by OCR output limit)' : ''}:${workbenchSource}\n\n${document.markdown}`
          agent.inject(createUserMessage({
            ...
            source: {
              ...
              plugin: 'mineru-ocr',
              form: 'notice',
              summary: boundContextSummary(`OCR document: ${document.name}`),
            },
          }))
        }
...
const MAX_DOCUMENT_CONTEXTS = 8
const MAX_DOCUMENT_NAME_CHARS = 256
const MAX_DOCUMENT_MARKDOWN_CHARS = 500_000
const MAX_DOCUMENT_SOURCE_ID_CHARS = 80
const MAX_DOCUMENT_MEDIA_TYPE_CHARS = 200

/** Enforce the document-context limits that are not expressible in the generated Remote type graph. */
function validateDocumentContexts(contexts: readonly import('./types.ts').PromptDocumentContext[] | undefined): void {
  if (contexts === undefined) return
  if (contexts.length > MAX_DOCUMENT_CONTEXTS) {
    throw new RemoteError('gateway/bad-request', `session.prompt accepts at most ${String(MAX_DOCUMENT_CONTEXTS)} document contexts`, {})
  }
  for (const context of contexts) {
    if (context.name.length < 1 || context.name.length > MAX_DOCUMENT_NAME_CHARS) {
      throw new RemoteError('gateway/bad-request', `document context name must contain 1-${String(MAX_DOCUMENT_NAME_CHARS)} characters`, {})
    }
    if (context.markdown.length < 1 || context.markdown.length > MAX_DOCUMENT_MARKDOWN_CHARS) {
      throw new RemoteError('gateway/bad-request', `document context markdown must contain 1-${String(MAX_DOCUMENT_MARKDOWN_CHARS)} characters`, {})
    }
    if (context.sourceId !== undefined
      && (context.sourceId.length < 1 || context.sourceId.length > MAX_DOCUMENT_SOURCE_ID_CHARS)) {
      throw new RemoteError('gateway/bad-request', `document context sourceId must contain 1-${String(MAX_DOCUMENT_SOURCE_ID_CHARS)} characters`, {})
    }
    if (context.sourceMediaType !== undefined
      && (context.sourceMediaType.length < 1 || context.sourceMediaType.length > MAX_DOCUMENT_MEDIA_TYPE_CHARS)) {
      throw new RemoteError('gateway/bad-request', `document context sourceMediaType must contain 1-${String(MAX_DOCUMENT_MEDIA_TYPE_CHARS)} characters`, {})
    }
  }
}
