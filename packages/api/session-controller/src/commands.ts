...
function validateDocumentContexts(contexts: readonly import('./types.ts').PromptDocumentContext[] | undefined): void {
  ...
  if (contexts.length > MAX_DOCUMENT_CONTEXTS) {
    reject('bad-request', `session.prompt accepts at most ${String(MAX_DOCUMENT_CONTEXTS)} document contexts`, {})
  }
  for (const context of contexts) {
    if (context.name.length < 1 || context.name.length > MAX_DOCUMENT_NAME_CHARS) {
      reject('bad-request', `document context name must contain 1-${String(MAX_DOCUMENT_NAME_CHARS)} characters`, {})
    }
    if (context.markdown.length < 1 || context.markdown.length > MAX_DOCUMENT_MARKDOWN_CHARS) {
      reject('bad-request', `document context markdown must contain 1-${String(MAX_DOCUMENT_MARKDOWN_CHARS)} characters`, {})
    }
    if (context.sourceId !== undefined
      && (context.sourceId.length < 1 || context.sourceId.length > MAX_DOCUMENT_SOURCE_ID_CHARS)) {
      reject('bad-request', `document context sourceId must contain 1-${String(MAX_DOCUMENT_SOURCE_ID_CHARS)} characters`, {})
    }
    if (context.sourceMediaType !== undefined
      ...
      reject('bad-request', `document context sourceMediaType must contain 1-${String(MAX_DOCUMENT_MEDIA_TYPE_CHARS)} characters`, {})
    }
  }
}
