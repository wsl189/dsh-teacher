...
`Session.prompt` accepts up to eight `PromptDocumentContext` rows in addition to visible text and images. The Host validates every filename, Markdown body, and optional source identifier and media type, then injects each document as a durable `mineru-ocr` plugin-authored user context immediately before the visible user prompt. The OCR text therefore stays outside the ordinary composer draft while remaining reconstructable from the Session log for every model request. Optional `sourceId` and `sourceMediaType` fields identify a browser-retained workbench source that the UI can reopen while that source remains available.
