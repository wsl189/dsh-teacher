...
除可见文本与图片外，`Session.prompt` 还接受最多八条 `PromptDocumentContext`。Host 会验证每个文件名、Markdown 正文以及可选的源标识与媒体类型，再在可见用户 prompt 之前把各文档注入为持久的 `mineru-ocr` 插件用户上下文。因此 OCR 文本不会进入普通 composer 草稿，但每次模型请求仍可从 Session 日志重建。可选的 `sourceId` 与 `sourceMediaType` 用于标识浏览器保留的工作台源文件，只要该源仍然可用，UI 就能重新打开它。
