...
When `includeImages` is true, Markdown extraction sets `return_images=true` and returns the selected document’s image data URLs as canonical base64 assets keyed by `images/<filename>`. Invalid payloads, conflicting targets, and accumulated image data beyond `maxResponseBytes` fail extraction. Other requests keep image return disabled.
