---
description: "自托管 MinerU OCR 提供方：通过 ctx.ocr 提供 Markdown 提取、栅格细节轮次与归一化 PDF 页面几何信息。"
...
本插件向 `ctx.ocr` 注册 `mineru` 提供方 id，并把上传文件发送到由部署方控制的 MinerU 同步 `/file_parse` 端点。
...
完成上述修复后，如果横向 PDF 页面开头的普通题号存在向前跳号，提供方会把来源页裁成独立的左、右半页 PDF 并再次解析。半页坐标会按比例缩放并平移回来源页面。恢复出的题组会补齐缺失题号；只有同题号原始题组跨过页面中线而恢复题组位于单栏时，后者才会替换前者。没有该信号的页面不会发起分栏恢复请求，因此一张可疑跨页只增加两次 MinerU 调用，无需把整份文档改为单栏解析。
...
- [OCR 运行时](../ocr/README.zh.md)——提供方选择与归一化结果所有权。
- [OCR 子系统](../../../docs/subsystems/ocr.zh.md)——共享提取与版面类型。
- [教师工作台](../../host/teacher-workbench/README.zh.md)——经过复核的课程表与试题切割 Consumer。
...
不发布运行时 invariant 配套插件；每次提供方调用都会验证网络响应和上传限制。

端点专用响应处理保留在本提供方内，并保持 `dsh-ocr` 暴露的提供方无关 OCR 类型不变。
