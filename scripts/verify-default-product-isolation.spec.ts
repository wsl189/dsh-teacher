...
describe('default product isolation', () => {
  it('allows only the teacher Web bundle to mount the official native computer provider', () => {
    const root = fixture()
    ...
    write(root, 'packages/experimental/computer-use-cua-driver-native/src/index.ts', 'export {}\n')
    write(root, 'packages/bundle/web-app/package.json', {
      name: web, dependencies: { [provider]: 'workspace:^' }, dsh: { bundle: { patch: './cordis.patch.yml' } },
    })
    write(root, 'packages/bundle/web-app/cordis.patch.yml', [{ insert: [{ name: provider }] }])
    write(root, profile, `export const PROFILE_TEMPLATES = { web: { bundles: ['${base}', '${web}'], patchReload: 'live' } }\n`
      + `export const DEFAULT_PROFILE_BUNDLES = ['${base}']\n`)
    expect(verifyDefaultProductIsolation(root).failures).toEqual([])
    write(root, 'packages/core/core/src/index.ts', `import '${provider}'\n`)
    expect(verifyDefaultProductIsolation(root).failures.join('\n')).toContain('packages/core/core/src/index.ts')
    manifest(root, 'packages/bundle/web-app/package.json', { dependencies: { [experimental]: 'workspace:^' } })
