...
\`${UNIVER_OFFICE_PACKAGE}\` ${distribution.pluginVersion} is Apache-2.0, but its executable closure also contains the packages below. Its external \`@univerjs-pro/cli-assets\` dependency supplies the commercial resource catalog. Its build script inlines the listed \`@univerjs-pro/*\` and \`@univer-cli/*\` build-time modules into the Host, Viewer, Gateway, worker, and render artifacts. This distribution excludes the Viewer and Client interface; the retained modifications are recorded in [runtime.patch](third-party/dsh-univer-office/runtime.patch) and [headless.patch](third-party/dsh-univer-office/headless.patch). Those modules retain their own terms; the wrapper's Apache-2.0 declaration does not relicense them, and the compiled tarball does not carry their individual package manifests or notices. [Univer's licensing guide](${UNIVER_COMMERCIAL_TERMS}) requires a valid Univer Pro commercial license for production use. Inclusion in this repository or an installer does not grant that license; every distributor and production operator must obtain all production and distribution rights required by Univer. A package identity, version, bundled-declaration digest, or platform-payload change requires another dependency, compatibility, terms, and notices review.
