...
it('preserves upstream PPT Master text while checking the owned provider', () => {
  const word = 'prove' + 'nance'
  expect(findConcreteTermViolations('packages/skill/skill-ppt-master/assets/ppt-master/README.md', word)).toEqual([])
  expect(findConcreteTermViolations('packages/skill/skill-ppt-master/src/index.ts', word)).toHaveLength(1)
})

describe('concrete terminology policy', () => {
  it('preserves only the two IANA types in the pinned IM bundle patch', () => {
    const file = 'patches/xmanrui-dsh-im@4.21.1.patch'
    const registered = `"application/${blockedTerm}+xml";"text/${blockedTerm}-notation"`
    expect(findConcreteTermViolations(file, registered)).toEqual([])
    expect(findConcreteTermViolations(file, `${registered};${blockedTerm}`)).toHaveLength(1)
    expect(findConcreteTermViolations('packages/example/src/index.ts', registered)).toHaveLength(1)
    expect(findConcreteTermViolations('patches/other.patch', registered)).toHaveLength(1)
  })
