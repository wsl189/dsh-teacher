...
const DESKTOP_PROVIDER = '@deepseek-ai/dsh-experimental-computer-use-cua-driver-native'
const desktopProviderOrigin = (origin: string): boolean => origin === '@deepseek-ai/dsh-web-app dependencies'
  || origin === 'packages/bundle/web-app/cordis.patch.yml'
  || origin.startsWith('packages/experimental/computer-use-cua-driver-native/')
...
export function verifyDefaultProductIsolation(root: string): ProductIsolationResult {
  ...
  const add = (pkg: Package, origin: string): void => {
    if (isExperimental(pkg) && !(pkg.manifest.name === DESKTOP_PROVIDER && desktopProviderOrigin(origin))) {
  ...
  const reference = (name: string, origin: string, owner?: Package): void => {
    ...
    if (packageName.startsWith(EXPERIMENTAL_PREFIX)
      && !(packageName === DESKTOP_PROVIDER && desktopProviderOrigin(origin))) {
  ...
  const scanEntries = (
    ...
    const visit = (entry: unknown): void => {
      ...
      if (typeof entry.name === 'string') {
        ...
        } else reference(entry.name, composedWeb && entry.name === DESKTOP_PROVIDER
          ? 'packages/bundle/web-app/cordis.patch.yml' : display(path), owner)
      }
