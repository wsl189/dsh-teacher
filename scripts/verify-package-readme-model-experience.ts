...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/core/agent-default-model': { kind: 'indirect', reason: 'The service supplies default and tool ModelSelections; request assembly and adapters own the model-visible request.' },
  ...
  'packages/client/ui-desktop-update': { kind: 'none', reason: 'Browser-side desktop updater control; registers nothing model-facing.' },
  'packages/client/ui-teacher-workbench': { kind: 'indirect', reason: 'Timetable import and question cutting invoke Host-owned children; the Host package owns their model-visible requests.' },
  'packages/ocr/ocr': { kind: 'indirect', reason: 'The extraction seam delegates model request rendering to its Consumers.' },
  'packages/ocr/ocr-mineru': { kind: 'indirect', reason: 'The MinerU backend delegates model request rendering to dsh-ocr Consumers.' },
  'packages/speech/speech': { kind: 'none', reason: 'The browser Remote returns user-owned draft text and registers no model surface.' },
  'packages/speech/speech-qq': { kind: 'none', reason: 'The QQ-configured backend returns text to browser Consumers and registers no model surface.' },
  ...
  'packages/client/ui-settings-models': { kind: 'indirect', reason: 'The browser row selects a tool model; product-owned background-agent consumers own each model-visible request.' },
  ...
  'packages/util/concurrency': { kind: 'indirect', reason: 'Only concurrency consumers own model-backed work and render its outcomes.' },
