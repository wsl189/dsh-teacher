...
const excludedPrefixes = ['vendor/', '.agents/notes/archived/', 'packages/skill/skill-ppt-master/assets/ppt-master/'] as const
...
export function findConcreteTermViolations(file: string, source: string): ConcreteTermViolation[] {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    // The bundled MIME database retains its two registered IANA media types.
    const maintained = file === 'patches/xmanrui-dsh-im@4.21.1.patch'
      ? line.replaceAll(`"application/${blockedTerm}+xml"`, '""').replaceAll(`"text/${blockedTerm}-notation"`, '""')
      : line
    if (containsBlockedTerm(maintained)) violations.push({ file, line: index + 1 })
  }
  return violations
}

function trackedFiles(repoRoot: string): string[] {
  const files = execFileSync('git', ['ls-files', '-z'], { cwd: repoRoot, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 })
    .split('\0')
    .filter(file => file !== '')
