...
describe('client bundle build faces', () => {
  ...
  it('applies dependency aliases only to the browser bundle', () => {
    const configs = clientBundle('@deepseek-ai/dsh-client-test', ['lib/types/index.js'], {
      clientAlias: { fflate: 'fflate/browser' },
    })({ env: { DSH_BUILD_FACE: 'client' } })
    const browser = configs.find(config => config.platform === 'browser')
    const node = configs.find(config => config.platform === 'node')

    expect(browser?.alias).toEqual({ fflate: 'fflate/browser' })
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    ...
    expect(resolveId('@deepseek-ai/dsh-concurrency')).toBeNull()
